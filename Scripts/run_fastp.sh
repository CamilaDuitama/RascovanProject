...
# $5 is folder where trimmed aMetagenome fastqfiles are gonna be located
...
while read -r line ;do
	...
	elif [ -f $2$accession"_1.fastq.gz" ] && [ ! -f $2$accession"_2.fastq.gz" ]; then

		fastp -i $2$accession".fastq.gz" -o $4$accession"_1.fastq.gz" -w 5

        elif [ -f $3$accession"_1.fastq.gz" ] && [ ! -f $3$accession"_2.fastq.gz" ]; then
		
		fastp -i $3$accession".fastq.gz" -o $5$accession"_1.fastq.gz" -w 5
